...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const phoneOpenBtn = document.querySelector('.btn-phone-open');
    if (phoneOpenBtn) {
        phoneOpenBtn.onclick = function(e) {
            e.preventDefault();
            desktopBlock.classList.add('hidden');
            mobileContent.style.display = 'block';
            setTimeout(() => initializeWebsite(), 300);
...
function moveNoButton() {
    ...
    if (!btn) return;
    // FIX: smaller range than before — the button now sits in normal
    // flow inside a max-width: 300px container instead of being
    // fixed to the full viewport, so large jumps would push it
    // outside its container or off-screen on narrow phones.
